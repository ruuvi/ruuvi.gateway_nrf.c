...
static bool filter_company_id(uint8_t* adv_data, uint8_t len) {
    if (m_adv_filter.active) {
        uint8_t *manuf_data = ble_advdata_parse(adv_data, len, BLE_GAP_AD_TYPE_MANUFACTURER_SPECIFIC_DATA);
        if (manuf_data) {
            uint16_t company_id;
...
static void set_scan_channel(uint8_t channel)
{
    if (channel >= 0 && channel <= 39) {
        ...
        NRF_LOG_DEBUG("Setting scanning channel: ch: %d, b: %d, bit: %d, mask: 0x%x", channel, b, bit, mask);
    } else {
...
static void scan_evt_handler(scan_evt_t const * p_scan_evt)
{
    ...
    case NRF_BLE_SCAN_EVT_NOT_FOUND: {
        ...
        if (filter_company_id(p_adv_report->data.p_data, p_adv_report->data.len)) {
            uart_send_broadcast(p_adv_report->peer_addr.addr, p_adv_report->data.p_data, p_adv_report->data.len, p_adv_report->rssi);
            led_blink();
        }
    } break;
...
void scan_init(void)
{
    ...
    memset(m_scan_param.channel_mask, 0, sizeof(m_scan_param.channel_mask)); //enable all primary and secondary channels
